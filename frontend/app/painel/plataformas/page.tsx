...
export default function PlataformasPage() {
  ...
  async function handleBulkDelete() {
    if (!window.confirm(`Excluir ${selected.size} plataforma(s)? Esta ação não pode ser desfeita.`)) return
    setBulkDeleting(true)
    try {
      await Promise.all(
        Array.from(selected).map((id) =>
          apiFetch(`${BACKEND_URL}/api/admin/plataformas/${id}/`, { method: 'DELETE' })
        )
      )
      setSelected(new Set())
      load()
  ...
  const load = useCallback(() => {
    ...
    apiFetch(`${BACKEND_URL}/api/admin/plataformas/?${params}`)
  ...
  function openEdit(item: Platform) {
    setEditing(item)
    setName(item.name)
  ...
  async function handleSave() {
    ...
    try {
      const url = editing
        ? `${BACKEND_URL}/api/admin/plataformas/${editing.id}/`
        : `${BACKEND_URL}/api/admin/plataformas/`
  ...
  async function handleToggle(item: Platform) {
    await apiFetch(`${BACKEND_URL}/api/admin/plataformas/${item.id}/`, {
      method: 'PATCH',
  ...
  async function handleDelete() {
    ...
    setDeleting(true)
    try {
      await apiFetch(`${BACKEND_URL}/api/admin/plataformas/${deleteTarget.id}/`, { method: 'DELETE' })
    ...
    <div>
      ...
      <div className="flex items-start justify-between mb-6">
        <div>
          ...
          <p className="text-sm text-zinc-500 mt-1">
            {total} {total === 1 ? 'plataforma cadastrada' : 'plataformas cadastradas'}
          </p>
        </div>
        <button
          onClick={openCreate}
      ...
      <div className="bg-zinc-900 border border-zinc-800 rounded-2xl overflow-hidden">
        ...
        {!loading && items.length === 0 && (
          <p className="px-6 py-12 text-center text-sm text-zinc-500">Nenhuma plataforma encontrada.</p>
        )}
      </div>
      ...
      {(
        <div className="flex items-center justify-between mt-4">
          <p className="text-xs text-zinc-500">
            Página {page} de {pages} &mdash; {total} plataformas
          </p>
        ...
        <DialogContent className="bg-zinc-950 border border-zinc-800 text-white max-w-sm">
          <DialogHeader>
            <DialogTitle className="text-base font-semibold text-white">
              {editing ? 'Editar Plataforma' : 'Nova Plataforma'}
            </DialogTitle>
          </DialogHeader>
      ...
      <Dialog open={!!deleteTarget} onOpenChange={(o) => !o && setDeleteTarget(null)}>
        <DialogContent className="bg-zinc-950 border border-zinc-800 text-white max-w-sm">
          <DialogHeader>
            <DialogTitle className="text-base font-semibold text-white">Excluir Plataforma</DialogTitle>
          </DialogHeader>
